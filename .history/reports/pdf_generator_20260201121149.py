...
def generate_pdf(data, feedback, photo=None, pie_values=None, pie_labels=None):
    ...
    pie_labels -> list of labels for pie chart (shown below the pie)
    """
    ...
    img = Image(img_path, width=95, height=95) if img_path else Spacer(95, 95)
    ...
    # Data Table
    # --------------------------
    table_data = [[k, v or ""] for k, v in data.items()]
    ...
    if pie_values and pie_labels:
        ...
        # Add labels below pie chart
        label_text = " | ".join(pie_labels)
        elements.append(Paragraph(f"<b>Colors:</b> {label_text}", styles["Normal"]))
        elements.append(Spacer(1, 25))
