...
def login_ui():
    ...
    if "registered_face" in st.session_state and st.session_state.registered_face is not None:
        ...
        # Sidebar inputs (read their current values)
            name = st.sidebar.text_input("Child Name", value=st.session_state.child_name, key="child_name")location = st.sidebar.text_input("Location", value=st.session_state.location, key="location")
