...
def login_ui():
    ...
    # ✅ Bind directly to session_state (VERY IMPORTANT)
    st.sidebar.text_input("Child Name", key="child_name")
    st.sidebar.text_input("Location", key="location")

    # -----------------------
    # Step 1: Registered Photo
    # -----------------------
    ...
    if st.sidebar.button("Capture Registered Photo"):
        if reg_ctx.video_processor and reg_ctx.video_processor.frame is not None:
            face = reg_ctx.video_processor.frame.copy()

            st.session_state.registered_face = face   # ✅ save permanently
    ...
    # Step 2: Verification
    # -----------------------
    if st.session_state.get("registered_face") is not None:

        st.sidebar.subheader("Step 2: Verify Face")
        st.sidebar.info("📸 Look at camera and click 'Verify Face'")
        ...
        if st.sidebar.button("Verify Face"):
            if live_ctx.video_processor and live_ctx.video_processor.frame is not None:
                ...
                if compare_faces(st.session_state.registered_face, live_face):
                    ...
                    # ✅ Now values already exist in session_state
