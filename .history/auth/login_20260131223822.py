...
def login_ui():
    ...
    name = st.sidebar.text_input("Child Name", value=st.session_state.child_name)
location = st.sidebar.text_input("Location", value=st.session_state.location)
    ...
    if st.sidebar.button("Capture Registered Photo"):
        if reg_ctx.video_processor and reg_ctx.video_processor.frame is not None:
            registered_face = reg_ctx.video_processor.frame.copy()
            st.sidebar.image(cv2.cvtColor(registered_face, cv2.COLOR_BGR2RGB), caption="Registered Photo")
