...
def login_ui():
    ...
    # Step 2: Verification
    # =========================
    if st.session_state.registered_face is not None:
        ...
        if st.sidebar.button("Verify Face"):
            ...
            if live_ctx.video_processor and live_ctx.video_processor.frame is not None:
                ...
                if compare_faces(st.session_state.registered_face, live_face):
                    ...
                    # FINAL VALIDATION
                    if st.session_state.child_name.strip() == "":
                        st.sidebar.error("Enter child name")
                        return

                    if st.session_state.location.strip() == "":
                        st.sidebar.error("Enter location")
                        return
