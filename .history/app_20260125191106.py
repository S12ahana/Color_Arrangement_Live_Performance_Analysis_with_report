...
if st.button("⚡ Analyze Snapshot (Left → Right)"):
    ...
    # 🔥 IMPORTANT: ignore invalid detections
    sorted_colors = sorted(
        [(c, pos) for c, pos in detected.items() if pos is not None],
        key=lambda x: x[1][0]  # left → right
    )
    ...
    with left:
        ...
        # ✅ Green box ONLY if color AND position match
    ...
    ax.axis("equal")  # Makes pie perfectly round
    ...
    st.write("Detected Order:", detected_order)
    ...
    pdf_path = generate_pdf(
        data,
        "🏆 Excellent" if accuracy == 100 else "⚡ Needs Improvement"
    )
